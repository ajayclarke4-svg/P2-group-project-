...
// Description: Allows admin to either log in or create an account.
...
// Signup validates password rules and stores encrypted password to txt file store in data.
    ...
    while (valid != 1) {
        ...
        scanf("%s", adminPtr->username);

        printf("Create Password: ");
        scanf("%s",adminPtr->password);
        ...
        for (int i = 0; adminPtr->password[i] != '\0'; i++) {
                for(int j = 0; adminPtr->password[i] != '\0'; j++){
...
void Admin_login(struct Admin *adminPtr){
    ...
    while (attempts > 0) {
        ...
        scanf("%s", collected_Username);

        printf("Enter Password: ");
        scanf("%s", collected_Password);
